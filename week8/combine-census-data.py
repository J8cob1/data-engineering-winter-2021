...
for county in counties:
    ...
    for state in states:
        ...
        entries.append([state, county.replace(" County", ""), total_population, total_income_per_capita, overall_poverty_percentage])

# Write all the values we've saved to a new csv
parsed_census_data = pandas.DataFrame(entries, columns=["State", "County", "Population", "IncomePerCapita", "Poverty"])
parsed_census_data.to_csv("data/parsed_census_data.csv", index=False)